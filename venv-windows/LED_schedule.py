...
ALARM_HOUR = 17; ALARM_MINUTE = 30 # time when the clock hits "zero" each morning
COUNTDOWN_TIME = 30*60 # Number of seconds to count down and change LEDs
BEDTIME_HOUR = 19; BEDTIME_MINUTE = 00 # time when clock goes back to red
...
def LED_countdown(color_new, color_old):
        new_leds = (1.0-(countdown/COUNTDOWN_TIME))*led_num
        transition = new_leds - int(new_leds)
        new_leds = int(new_leds)

        for i in range(new_led):
